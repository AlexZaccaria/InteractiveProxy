...
function FilterRules({ onRulesChanged, filterMode = 'ignore', onFilterModeChange, filterRulesEnabled = true, onFilterRulesModeChange, showConfirm }) {
  ...
  const fetchBypassPatterns = useCallback(async () => {
    try {
      ...
      const response = await fetch(buildApiUrl(`/api/filters?${params.toString()}`));
  ...
  const fetchSuggestions = useCallback(async () => {
    try {
      ...
      const response = await fetch(buildApiUrl('/api/filters/suggestions'));
      const data = await response.json();
  ...
  const submitPattern = useCallback(async (pattern) => {
    ...
    try {
      setIsLoading(true);
      setError(null);
      const response = await fetch(buildApiUrl('/api/filters'), {
        method: 'POST',
              ...
              >
                ...
                <div className="flex items-center space-x-2 flex-shrink-0">
                  <button
                    ...
                    disabled={activeRuleId === rule.id && isLoading}
                  ...
                  >
                    {activeRuleId === rule.id && isLoading ? (
                      <Spinner size="sm" />
                    ) : (
                  ...
                  <button
                    ...
                    disabled={activeRuleId === rule.id && isLoading}
                    className="inline-flex items-center justify-center gap-2 px-3 h-8 bg-red-600/20 hover:bg-red-600/30 text-red-400 border border-red-600/30 rounded-lg transition-colors text-xs font-medium disabled:opacity-50 disabled:cursor-not-allowed"
                    title="Remove"
                  >
                    {activeRuleId === rule.id && isLoading ? (
                      <Spinner size="sm" />
